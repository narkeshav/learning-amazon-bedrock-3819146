...
st.markdown("<h1 style='text-align: center;'>📖 Multi-PDF Query Bot</h1>", unsafe_allow_html=True)
...
# Add a PDF selector in the sidebar
pdf_files = {
    "Social Media Training": "03_06b/social-media-training.pdf",
    "What is NLP": "03_06b/What is NLP_ - Natural Language Processing Explained - AWS.pdf"  # Add more PDFs here
}
selected_pdf = st.sidebar.selectbox("Select a PDF", list(pdf_files.keys()))

# Load the selected PDF
vectorstore_faiss = config_vector_db(pdf_files[selected_pdf])
st.markdown(f"<h3 style='text-align: center;'>Querying: {selected_pdf}</h3>", unsafe_allow_html=True)
...
logger.info(f"Vector database configured with file: {selected_pdf}")
